...
if(!isMobile){
	...
	(function avoidFOUC(){
		window.addEventListener('load', function(){
			enableEasing();
			window.addEventListener('resize', tempDisableEasing);
		});

		function tempDisableEasing(){
			disableEasing();
			enableEasing();
		}

		const disableEasing = _debounce(function(){
		...
		const enableEasing = _debounce(function(){
			document.body.classList.remove('preload');
		}, 250);
	})();

	(function initVideoController(){
		const vc = videoController('.video', '.video-controllers');
